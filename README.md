...
| `cd`     | <ul><li>No args → `$HOME` (error if unset).</li><li>`-` → `$OLDPWD` (error if unset); like bash, the resolved path is printed to stdout.</li><li>Maintains logical `PWD`, preserving symlink path components and resolving `.`/`..` lexically before `chdir`.</li><li>Rejects `cd a b` with "too many arguments" and leaves the directory unchanged.</li><li>After a successful `chdir`, sets `OLDPWD` to the previous logical `PWD` and `PWD` to the new logical path.</li></ul> |
| `pwd`    | <ul><li>Prints the shell-maintained logical `PWD` followed by a newline.</li><li>If `PWD` is unavailable, falls back to `getcwd()` and reports `strerror(errno)` on failure.</li></ul> |
